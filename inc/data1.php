...
**/
	...
	$items 		= json_decode($_POST["Cashname"]);
	$quantity 	= json_decode($_POST["Cashquantity"]);
	$staff 		= json_decode($_POST["Cashstaff"]);
	$branch 	= json_decode($_POST["Cashbranch"]);

	for ($i = 0; $i < count($items); $i++) {
		if(($items[$i] != "")){
  			$date     =  date('d/m/Y');
  			$time     =  date('h:m:s a');
  			$create   = $user->cashEndOfTheDay($items[$i], $quantity[$i], $branch[$i], $staff[$i], $date, $time);
		    if ($create) {
		      print "successfully added case of the day";
		    }else{
